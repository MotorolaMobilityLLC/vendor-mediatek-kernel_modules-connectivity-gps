	...
	enum mbr2gnss_reason reason, struct gnss2mbr_lp_data *p)
{
	...
	if ((ret == DATA_OK_END) || (ret == DATA_OK_AGAIN)) {
		p->hdr.data_size = sizeof(*p);
		p->hdr.major_ver = LP_DATA_MAJOR_VER;
		p->hdr.minor_ver = LP_DATA_MINOR_VER;

		/* p->dump_ts    = lp_data.dump_ktime_ms;
		 * p->dump_index = lp_data.dump_index;
		 */
		p->ap_resume_ts     = lp_data.dump_ktime_ms;
		p->ap_resume_index  = lp_data.dump_index;
	...
	enum mbr2gnss_reason reason, struct gnss2mbr_mcu_data *p)
{
	...
	if ((ret == DATA_OK_END) || (ret == DATA_OK_AGAIN)) {
		p->hdr.data_size = sizeof(*p);
		p->hdr.major_ver = MCU_DATA_MAJOR_VER;
		p->hdr.minor_ver = MCU_DATA_MINOR_VER;
