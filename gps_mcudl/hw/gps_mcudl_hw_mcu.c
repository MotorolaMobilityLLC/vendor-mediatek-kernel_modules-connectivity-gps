...
/* Poll BGF_ON_STATUS */
	...
	GDL_HW_CHECK_BGF_IP_VER(&poll_okay, &poll_ver_v06x);
	if (!poll_okay) {
...
{
	/*nee dump remap cr(only rw in atf donmain)*/
	...
	if (power_ctrl) {
		if (gps_mcu_common_on_flag == 0)
			arm_smccc_smc(MTK_SIP_KERNEL_GPS_CONTROL, SMC_GPS_COMMON_ON_SET_FLAG_OPID,
					0xdb9db9, power_ctrl, 0xdb8db8, 0, 0, 0, &res);
